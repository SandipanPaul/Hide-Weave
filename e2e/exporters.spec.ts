...
import { signInAtExporters, uniqueName } from "./helpers";
import { cleanupE2ERows } from "./db-cleanup";

/**
 * Milestone 5, part A: exporter CRUD against the real database.
 *
 * Rows created here are named "E2E …" and removed in teardown.
 */
test.describe("exporters", () => {
  test.beforeEach(async ({ page }) => {
    cleanupE2ERows();
    await signInAtExporters(page);
  });
  ...
  async function addExporter(page: import("@playwright/test").Page, fields: Record<string, string>) {
  ...
  test("adds an exporter, accepting a bare domain as its website", async ({ page }) => {
    const name = uniqueName("E2E Leather Co");
    await addExporter(page, {
      "Company name": name,
  ...
  test("treats the same site written differently as one exporter", async ({ page }) => {
    const first = uniqueName("E2E Klasse");
    await addExporter(page, { "Company name": first, Website: "https://klasseleather.example.com" });
  ...
  test("refuses a second exporter with the same name", async ({ page }) => {
    const name = uniqueName("E2E Trio Group");
    await addExporter(page, { "Company name": name });
    ...
    await expect(dialog.getByText("An exporter with this name already exists.")).toBeVisible();
  });

  test("edits an exporter in place", async ({ page }) => {
    const name = uniqueName("E2E Dugros");
    await addExporter(page, { "Company name": name });

    await page.getByLabel("Search exporters").fill(name);
    await expect(page.getByText(/of 1 exporters/)).toBeVisible();
    await page.getByRole("link", { name }).click();
